...
- `demos/<Name>Demo.tsx` — Canvas-плейграунд для секции `## Демо` в MDX (только для props-driven компонентов без центральных колбеков; см. [`.claude/rules/docs-structure.md`](https://github.com/)).
...
Разные комбинации пропсов проверяются через `gotoStory(id, args)` — отдельную story «под каждую комбинацию» заводить не нужно.
...
| **Skills** | `.claude/skills/*.md` | Агент **сам решает** активировать, когда задача матчится по описанию | Пошаговые воркфлоу со своим контекстом и подзадачами |
