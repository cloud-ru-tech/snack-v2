...
├── tooling/tsconfig/              # Базовые tsconfig шаблоны
├── scripts/
...
`styles.module.scss` + токены `@cloud-ru/figma-variables`:
...
2. регистрирует ссылки в `tsconfig.json`, `packages/tsconfig.esm.json`, `packages/tsconfig.cjs.json`;
3. добавляет alias в `apps/storybook/.storybook/main.ts` и `apps/docs/astro.config.mjs` (между маркерами `<add-package:aliases>`);
...
4. Skill `component-story-set` — Playground + VisualMatrix + baselines.
5. Skill `component-e2e-tests` — spec-файлы по tier'у.
6. Skill `component-docs` — `docs/index.mdx`, `demos/`, `FIGMA_<NAME>`.
7. Skill `component-validation-loop` — пройти цикл до зелёного.
8. `/make-commit`.

## Дизайн-токены

Визуальные константы — пакет `@cloud-ru/figma-variables`, установлен в монорепо.
