...
Реальная раскладка пакета — **nested по компоненту**, даже если компонент один. Подробности — в [`.claude/rules/package-src-structure.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/package-src-structure.md) и [`.claude/rules/reference-package-anatomy.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/reference-package-anatomy.md).
...
- `demos/<Name>Demo.tsx` — Canvas-плейграунд для секции `## Демо` в MDX (только для props-driven компонентов без центральных колбеков; см. [`.claude/rules/docs-structure.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/docs-structure.md)).
...
Правило: [`.claude/rules/packages-deps.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/packages-deps.md).
...
Полные правила: [`.claude/rules/stories-standard.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/stories-standard.md). Каждый компонент получает минимум **два** файла.
...
Каждая дополнительная story обязана проходить **«Критерий обоснованности артефакта»** из [`.claude/rules/complexity-tiers.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/complexity-tiers.md) (3 условия: проверяет что-то новое из публичной поверхности, не выражается через `args`/StoryTable/play, не дублирует другой слой). При переезде story между корнем и подпапкой обязательно обновить story IDs в `packages/<pkg>/__test__/<Name>/helpers.ts` — иначе e2e получит 404.

### Trigger-based компоненты

Modal, drawer, popover, dropdown, tooltip, toaster и любые dialog-like / portal-компоненты следуют отдельному правилу [`.claude/rules/trigger-based-stories.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/trigger-based-stories.md): `open` не выводится в args (живёт в локальном `useState` render-компонента), триггер — `Button` из `@ds/button` с `data-test-id={TEST_IDS.triggerOpen}`, `parameters.layout: 'fullscreen'`, конфликты значений между осями разрешаются runtime + `<DemoWarning>`, а не через `if:`.
...
Главная страница и сайдбар группируют пакеты по **префиксу имени**. Конфиг — [`apps/docs/src/config/domains.ts`](https://github.com/cloud-ru-tech/snack-v2/blob/master/apps/docs/src/config/domains.ts):
...
Внутри домена пакеты разбиты на категории — под-группы сайдбара доки и Storybook, секции главной и разделы `/llms.txt`: домен → категория → пакет. Категория задаётся вручную в [`apps/docs/src/config/categories.ts`](https://github.com/cloud-ru-tech/snack-v2/blob/master/apps/docs/src/config/categories.ts) (`CATEGORIES_BY_DOMAIN`); домен без записи рендерит пакеты плоско.
...
Полный шаблон MDX-страницы — [`.claude/rules/docs-structure.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/docs-structure.md).
...
Тесты живут в **трёх слоях** ([`.claude/rules/e2e-testing-standard.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/e2e-testing-standard.md)). Каждый слой решает свою задачу и не дублирует другие.
...
Полные правила: [`.claude/rules/e2e-testing-standard.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/e2e-testing-standard.md). Тесты **живут внутри пакета** — `packages/<pkg>/__test__/<Name>/*.spec.ts`. Fixtures импортируются из корневого `playwright/`.
...
- **`visual.spec.ts`** — всегда. Покрытие — по [`.claude/rules/visual-regression-standard.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/visual-regression-standard.md).
...
Baseline'ы лежат **рядом со спеком** — `packages/<pkg>/__test__/<Name>/__snapshots__/<arg>-<projectName>.png`. Снимки только на `chrome`, имена без префикса пакета (`visual-matrix.png`, не `button-visual-matrix.png`). Правила — [`.claude/rules/visual-regression-standard.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/visual-regression-standard.md).
...
Типовая ситуация: в приложении обнаружился баг компонента, правку нужно проверить на живом сценарии — но ждать CI, выпускать preview-версию и потом её отзывать долго. Для этого есть `ds-link`: пакеты собираются локально и доставляются приложению **под теми же именами, что в реестре** (`@ds/button` → `@sbercloud/snack-v2-button`), поэтому в его коде ничего менять не нужно. Подходит любому потребителю пакетов — микрофронту, монолитному сервису, песочнице.
...
- **Overrides подменяют зависимость, но не создают её.** Если пакет в приложении ещё не используется, сначала `pnpm add @sbercloud/snack-v2-<pkg>`.
...
- PR на ревью — открывайте Pull Request в репозитории на [GitHub](https://github.com/cloud-ru-tech/snack-v2).
...
Правила работы с Figma — [`.claude/rules/figma-integration.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/figma-integration.md) и [`.claude/rules/figma-to-code.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/figma-to-code.md).
...
Селективные команды для итераций (lint/build:pkg/typecheck/тесты по одному пакету) — см. [`.claude/rules/fast-build-commands.md`](https://github.com/cloud-ru-tech/snack-v2/blob/master/.claude/rules/fast-build-commands.md).
