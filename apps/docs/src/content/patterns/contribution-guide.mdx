...
`rendering.spec.ts`, `interaction.spec.ts`, `keyboard.spec.ts`, `polymorphism.spec.ts`, `visual.spec.ts`. Выбор набора — по tier'у ([`.claude/rules/complexity-tiers.md`](https://github.com/)).
...
- **Шаги:**
  ...
  2. `pnpm add-package` — скаффолд пакета. Делается **до** генерации плана, чтобы план опирался на реальную раскладку.
...
- **Префикс пакета:** **обязательно** `uikit-product-*` (например, `uikit-product-table`, `uikit-product-filter-bar`). Это разводит пакеты Product-кита и Snack-кита по разным неймспейсам.
...
Пакеты портируются из соседнего репо `storybook/` (`@design-system/*` → `@ds/*`) и из legacy-скоупов `@snack-uikit/*` / `@cloud-ru/*`. Конкретные рецепты под каждый источник — выше, в [`## Сценарии разработки`](#сценарии-разработки). Ниже — общий Claude-Code-флоу, на который опираются оба миграционных сценария.
...
<br /><a href='https://www.figma.com/community/plugin/1600446810191317919' target='_blank' rel='noopener noreferrer'>Figma community plugin</a>
...
Селективные команды для итераций (lint/build:pkg/typecheck/тесты по одному пакету) — см. [`.claude/rules/fast-build-commands.md`](https://github.com/).
