...
- **Оси — в React-контексте.** Значения `colorScheme`, `brand`, `platform`, `density`, `acrylic` живут в контексте `@ds/theme`, а не в пропах каждого компонента. Источник — один `RootThemeProvider` в корне приложения.
...
function App({ children }: { children: ReactNode }) {
  ...
  return (
    <RootThemeProvider value={{ colorScheme, brand: 'cloudConsole', density: 'comfort' }} rootRef={htmlRef}>
      {children}
    </RootThemeProvider>
  );
}
...
<ChildThemeProvider value={{ brand: 'snackUI' }}>{section}</ChildThemeProvider>;
```
...
| Бренд | `BRAND` | `cloudConsole` / `giga-id` / `gitverse` / `snackUI` / `hrBlue` … `hrYellow` / `siteBlue` … `siteYellow` → `sn-cloudConsole` … |
| Платформа | `PLATFORM` | `webDesktop` / `webMobile` → `sn-webDesktop` … |
| Плотность | `DENSITY` | `comfort` / `compact` / `spacious` → `sn-comfort` … |
| Акрил (blur-материал) | — (`boolean`) | `true` → `sn-yes`, `false` → `sn-no` |

Базовые слои (`sn-base-styles`, `sn-figmaStyles`, `sn-components`) и платформа эмитятся всегда — вместе с осями они образуют полный набор на границе.

Бренды HR-портала и сайта заданы финальными цветовыми вариантами:

- `hrBlue`, `hrGraphite`, `hrGreen`, `hrPurple`, `hrYellow` — HR-портал.
- `siteBlue`, `siteGraphite`, `siteGreen`, `sitePurple`, `siteYellow` — сайт.

Родительские слои `hrPortal` и `site` существуют только в Figma и в сборку токенов не попадают.

### Платформа

Платформа задаёт размеры, типографику и отступы, из которых плотность берёт свои значения, поэтому класс платформы всегда стоит рядом с классом плотности. Без значения ставится `webDesktop`.

Тема не читает раскладку `@ds/adaptive` сама. Приложение передаёт согласованные значения в оба провайдера:

```tsx
import { AdaptiveProvider, isMobileLayout } from '@ds/adaptive'
...
<AdaptiveProvider layoutType={layoutType}>
  <RootThemeProvider value={{ colorScheme, brand, platform, density }}>{app}</RootThemeProvider>
</AdaptiveProvider>;
```

`webMobile` соответствует только `layoutType: 'mobile'`: `tablet` / `desktopSmall` / `desktop` — `webDesktop`, как ветвление `isMobileLayout` в адаптивных компонентах.
...
```tsx
// Всё приложение — cloudConsole / comfort; одна секция — snackUI, остальные оси от родителя.
<RootThemeProvider value={{ brand: 'cloudConsole', density: 'comfort', colorScheme }}>
  <App />

  <ChildThemeProvider value={{ brand: 'snackUI' }}>
    {/* brand=snackUI, density=comfort и colorScheme унаследованы */}
    <PromoSection />
  </ChildThemeProvider>
</RootThemeProvider>;
```
...
- [Adaptive](/components/adaptive) — раскладка `layoutType`, согласованная с платформой темы.
