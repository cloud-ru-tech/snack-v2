...
export function useScrollContainer(container: RefObject<HTMLElement | null>): ScrollContainerControl {
  ...
  useLayoutEffect(() => {
    ...
    // Первичная проверка и после layout (важно для вертикального скролла)
    recheck();
...
export function useFocusControl(children: ReactElement<{ value: string; disabled?: boolean }>[]) {
  return useMemo(() => {
    const arrayOfIds = Children.map(children, (child: ReactElement<{ value: string; disabled?: boolean }>) => {
    ...
    return [getterElementWithShift(-1), getterElementWithShift(+1)] as const;
  }, [children]);
}
