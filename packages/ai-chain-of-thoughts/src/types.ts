...
export type AiChainOfThoughtsOwnProps = {
   ...
   */
  broken?: boolean;
  /** Длительность рассуждения в секундах. Форматируется в д/ч/м/с. */
  duration?: number;
  /** Подпись заголовка. По умолчанию «Размышляю» / «Размышлял» — по `inProgress`. */
  label?: ReactNode;
  /**
   * Иконка слева от подписи в активном состоянии. По умолчанию — GigaChat, `null` скрывает.
   * Ожидается SVG-иконка: из `@ds/icons`, `AiIconGiga` или `Sun`; при включённом `shimmer`
   * она заливается волной вместе с подписью.
   */
  icon?: ReactNode;
  /** Shimmer на подписи «Размышляю» в активном состоянии. По умолчанию `true`. */
  shimmer?: boolean;
...
export type AiChainOfThoughtsHeadlineOwnProps = {
   ...
   * нулевые единицы опускаются, секунды показываются всегда).
   */
  duration?: number;
  /** Подпись заголовка. По умолчанию «Размышляю» / «Размышлял» — по `inProgress`. */
  label?: ReactNode;
