...
| `brand` | `"brandA"` \| `"brandB"` \| `"brandC"` \| `"brandD"` | — | Бренд — `sn-brandA` … |
...
`useApplyCustomTheme({ color, scope })` инжектит правило, скоупленное на `#brand-hook-scope` — результат применяется ко всем компонентам внутри поддерева. Без `scope` правило глобальное (покрывает и порталы).
