...
- **Реактивное значение через `useSyncExternalStore`** — статичный проп оборачивается в стор, внешний реактивный источник (стор контейнера) передаётся напрямую; изменение доходит до подписчиков без перерендера провайдера.
...
- Значение приходит либо пропом (статично, SSR), либо из внешнего реактивного стора (RxJS/spa-core).
...
Когда значение приходит из внешнего реактивного источника (стор контейнера single-spa). Обновление стора доходит до всех подписчиков во всех микрофронтах без перерендера провайдера.
...
// Адаптер внешнего источника (RxJS-subject контейнера) в ExternalStore:
const store: ExternalStore<LayoutType> = {
  subscribe: onChange => container.layoutType.subscribe(onChange),
  getSnapshot: () => container.layoutType.get(),
};
