...
`InfoRow` адаптивен сам по себе — на mobile (раскладка из `AdaptiveProvider`) он переключается в вертикальный формат, отдельный компонент для этого не нужен.
...
- **mobile** — вертикальный формат: метка и значение друг под другом, карточка со скруглением и разделителями.
