...
export type WelcomeTourProps = {
  ...
  buttons?: TourButton[];
  /**
   * Показывать ли индикатор прогресса. Он и так появляется только у тура длиннее одного
   * шага; `false` убирает его вместе с озвучкой позиции для скринридера — это нужно там,
   * где часть шагов может отвалиться на лету (цели нет на странице, шаг пропускается),
   * и «2 из 5» окажется неправдой.
   * @default true
   */
