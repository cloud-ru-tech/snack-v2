...
import { buildToggleGroupStory, ROOT_SELECTOR, SCREENSHOT_OPTS } from '../_shared/helpers';
...
test.describe('ToggleGroup — visual regression', () => {
  ...
  test('static — default playground', async ({ page, gotoStory }) => {
    ...
    await expect(page.locator(ROOT_SELECTOR)).toHaveScreenshot('default.png', SCREENSHOT_OPTS);
  });
  ...
  test('state — single with selection', async ({ page, gotoStory, getByTestId }) => {
    ...
    await waitForFonts(page);
    await expect(page.locator(ROOT_SELECTOR)).toHaveScreenshot('single.png', SCREENSHOT_OPTS);
  });
  ...
  test('state — multiple with selection', async ({ page, gotoStory, getByTestId }) => {
    ...
    await waitForFonts(page);
    await expect(page.locator(ROOT_SELECTOR)).toHaveScreenshot('multiple.png', SCREENSHOT_OPTS);
  });
});
