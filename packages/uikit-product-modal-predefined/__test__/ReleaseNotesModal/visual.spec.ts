...
import { buildStoryOptions, RELEASE_NOTES_MODAL_STORIES, VM_TRIGGER_TEST_ID } from './helpers';

test.describe('ReleaseNotesModal — visual regression', () => {
  ...
  test('content states', async ({ page, gotoStory, getByTestId, waitForFonts }) => {
    ...
    for (const state of ['data', 'one', 'noData', 'error', 'loading']) {
      await gotoStory(buildStoryOptions(undefined, RELEASE_NOTES_MODAL_STORIES.visualMatrix));
