...
/**
 * Шапка окна вместе с верхней safe-area.
 *
 * Отступ лежит здесь, а не во фрейме: иначе его теряет ручная композиция из
 * `ModalCustom.Header/Body/Footer`. На `sheet` не нужен — там свой `padding-top` в токенах.
 */
...
/**
 * Футер окна вместе с зазором от `Body` (нижняя safe-area).
 *
 * Кнопки-слоты раскладывает `FooterActions`. `children` — для нестандартного содержимого:
 * раскладку в этом случае обеспечивает потребитель, `contentWrapper` выравнивает детей по левому краю.
 */
