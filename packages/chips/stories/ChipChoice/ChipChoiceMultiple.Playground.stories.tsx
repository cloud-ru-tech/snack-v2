...
type StoryProps = ChipChoiceCustomStoryProps & ComponentProps<typeof ChipChoice.Multiple>;

const Template = (args: StoryProps) => {
  const controlledArgs = useControlledStoryArgs<StoryProps>({ defaultValue: [] });

  return (
    <DemoPage>
      <DemoPanel>
        <DemoTitle>Playground (Multiple)</DemoTitle>
        <DemoHint>Чип-фильтр с выпадающим множественным выбором.</DemoHint>
        <DemoActions align='center'>
          <div className={styles.resizableWrapper}>
            <ChipChoice.Multiple {...args} {...controlledArgs} />
          </div>
        </DemoActions>
      </DemoPanel>
    </DemoPage>
...
const meta: Meta<typeof ChipChoice.Multiple> = {
  title: 'Components/Chips/ChipChoice/Multiple',
