...
2. **normalizeSvgColors** (после записи файла в svgs-fixed):
   ...
   - **stroke-width:**
     - `stroke-width="1.5"` и `stroke-width="1.5px"` (с любыми пробелами) → `stroke-width="var(--sn-density-icon-strokeWeight-s)"`.
...
- **Толщина обводки не по дизайну:** в fixIcons все `stroke-width="1.5"` и `1.5px` заменяются на `var(--sn-density-icon-strokeWeight-s)`. Убедитесь, что эта переменная подключена в приложении (например, из @ds/figma-variables).
