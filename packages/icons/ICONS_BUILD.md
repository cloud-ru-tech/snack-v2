...
- **Толщина обводки не по дизайну:** в fixIcons все `stroke-width="1.5"` и `1.5px` заменяются на `var(--sn-density-size-icon-strokeWeight-s)`. Убедитесь, что эта переменная подключена в приложении (например, из @ds/figma-variables).
