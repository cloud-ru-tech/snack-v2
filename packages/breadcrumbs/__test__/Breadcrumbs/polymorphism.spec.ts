...
test.describe('Breadcrumbs — polymorphism', () => {
  test('renders anchor with href when href is supplied', async ({ gotoStory, getByTestId }) => {
    await gotoStory(buildStoryOptions({ storyUrl: true }));

    const crumbs = getByTestId(TEST_IDS.crumb);
    await expect(crumbs.nth(0).locator('a')).toHaveAttribute(
      'href',
    ...
    await expect(crumbs.nth(2).locator('a')).toHaveAttribute(
      'href',
    ...
    await expect(crumbs.nth(3).locator('a')).toHaveAttribute(
      'href',
