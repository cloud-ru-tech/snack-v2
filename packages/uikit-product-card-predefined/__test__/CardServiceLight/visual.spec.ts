...
test.describe('CardServiceLight — visual regression', () => {
  ...
  test('visual-matrix', async ({ page, gotoStory, waitForFonts }) => {
    await gotoStory(buildStoryOptions(undefined, CARD_SERVICE_LIGHT_STORIES.visualMatrix));
    await waitForFonts();
  ...
  test('interaction states (default × hover × focus)', async ({ page, gotoStory, waitForFonts, getByTestId }) => {
    ...
    await assertInteractionStatesSnapshot(page, {
      target: getByTestId(TEST_IDS.cardServiceLight),
      padding: 8,
    });
  });
});
