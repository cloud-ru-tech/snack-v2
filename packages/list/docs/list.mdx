...
import { CollapseExpandIcon } from '../demos/examples/CollapseExpandIcon'
import CollapseExpandIconSrc from '../demos/examples/CollapseExpandIcon.tsx?raw'
...
<Example
  title='Раскрытие только по шеврону'
  description="collapse.toggleOn: 'expandIcon' — строка-ссылка ведёт на роут, раскрытие переключает шеврон."
  code={CollapseExpandIconSrc}
>
  <CollapseExpandIcon client:visible />
</Example>
...
- [ReorderableList](/components/list/reorder) — тот же список, но с drag&drop-переупорядочиванием строк.
