...
- [**ItemContent**](/components/list/item-content) — каноничная разметка содержимого item: `option` (заголовок), `caption` (мета справа), `description` (подпись снизу). Используется как значение `item.content`.
