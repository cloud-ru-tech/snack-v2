...
title: ItemContent
package: '@ds/list'
description: Каноничная разметка содержимого элемента списка — label / caption / description с единым truncate-поведением.
order: 4
---
...
<Example
  ...
  description='item.content = { label, caption, description } — List оборачивает в ItemContent сам.'
  code={BasicListSrc}
>
...
<Example
  ...
  description='label + caption (мета справа) + description (подпись снизу) в одном item.'
...
<Example
  ...
  description='truncate.label/description задают число строк, variant — место многоточия (end/middle). В узком контейнере длинный текст обрезается.'
