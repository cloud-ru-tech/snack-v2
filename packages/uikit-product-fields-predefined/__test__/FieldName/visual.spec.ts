...
test.describe('FieldName — visual regression', () => {
  ...
  test('visual-matrix', async ({ page, gotoStory, waitForFonts }) => {
    await gotoStory(buildStoryOptions(undefined, FIELD_NAME_STORIES.visualMatrix));
    await waitForFonts();
  ...
  test('interaction states (default × hover × focus)', async ({ page, gotoStory, getByTestId, waitForFonts }) => {
    ...
    await waitForFonts();
    await assertInteractionStatesSnapshot(page, { target: getByTestId(TEST_IDS.fieldName) });
  });
});
