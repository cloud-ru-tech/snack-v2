...
Компактная метка. По умолчанию `<span>`; становится `<a>` при `href`; показывает кнопку удаления при `onDelete`. Режимы взаимоисключающие: тег-ссылка кнопку удаления не отображает — для неё `onDelete` игнорируется.
