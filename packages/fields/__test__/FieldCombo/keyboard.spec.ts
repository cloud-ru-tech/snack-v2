...
import { buildStoryOptions, CLEAR_BUTTON_TEST_ID, FIELD_COMBO_STORIES, STORY_TEST_IDS, TEST_IDS } from './helpers';

test.describe('FieldCombo — keyboard navigation', () => {
  test('arrow nav: input ↔ clear button', async ({ gotoStory, getByTestId }) => {
    await gotoStory(buildStoryOptions());
    const input = getByTestId(TEST_IDS.fieldComboInput);
    await input.click();
  ...
  test('arrow nav: input ↔ elementBefore (ArrowLeft at start)', async ({ gotoStory, getByTestId }) => {
    ...
    const input = getByTestId(STORY_TEST_IDS.fieldCombo.droplistBeforeRoot).getByTestId(TEST_IDS.fieldComboInput);
    const beforeButton = getByTestId(STORY_TEST_IDS.fieldCombo.droplistBeforeButton);
  ...
  test('arrow nav: input → elementAfter (ArrowRight at end)', async ({ gotoStory, getByTestId }) => {
    await gotoStory(buildStoryOptions(undefined, FIELD_COMBO_STORIES.withDroplist));
    const input = getByTestId(STORY_TEST_IDS.fieldCombo.droplistAfterRoot).getByTestId(TEST_IDS.fieldComboInput);
    const afterButton = getByTestId(STORY_TEST_IDS.fieldCombo.droplistAfterButton);
  ...
  test('arrow nav: 3-hop input → clear → elementAfter, then back to clear', async ({ gotoStory, getByTestId }) => {
    await gotoStory(buildStoryOptions(undefined, FIELD_COMBO_STORIES.rovingScene));
    const root = getByTestId(STORY_TEST_IDS.fieldCombo.rovingSceneRoot);
    const input = root.getByTestId(TEST_IDS.fieldComboInput);
  ...
  test('elementAfter: ArrowDown opens the embedded droplist', async ({ gotoStory, getByTestId }) => {
    ...
    await expect(getByTestId(`${STORY_TEST_IDS.fieldCombo.droplistItem}-1`)).toBeVisible();
  });
});
