...
test.describe('FieldCombo — visual regression', () => {
  ...
  test('visual-matrix', async ({ page, gotoStory, waitForFonts }) => {
    await gotoStory(buildStoryOptions(undefined, FIELD_COMBO_STORIES.visualMatrix));
    await waitForFonts();
  ...
  test('field interaction states', async ({ page, gotoStory, waitForFonts, getByTestId }) => {
    ...
    await assertInteractionStatesSnapshot(page, {
      target: getByTestId(TEST_IDS.fieldCombo),
      hoverTarget: getByTestId(TEST_IDS.fieldComboInput),
  ...
  test('element-button interaction states', async ({ page, gotoStory, waitForFonts, getByTestId }) => {
    await gotoStory(buildStoryOptions(undefined, FIELD_COMBO_STORIES.elementButtonStates));
    await waitForFonts();
    await assertInteractionStatesSnapshot(page, {
      target: getByTestId(TEST_IDS.fieldComboElementButton),
      includePressed: true,
    });
  });
  ...
  test('open-droplist (before × after)', async ({ page, gotoStory, waitForFonts, getByTestId }) => {
    ...
    for (const variant of ['before', 'after'] as const) {
      const root = getByTestId(STORY_TEST_IDS.fieldCombo[`droplist${variant === 'before' ? 'Before' : 'After'}Root`]);
      const button = getByTestId(
        STORY_TEST_IDS.fieldCombo[`droplist${variant === 'before' ? 'Before' : 'After'}Button`],
      );
      await button.click();
      const firstItem = getByTestId(`${STORY_TEST_IDS.fieldCombo.droplistItem}-1`);
      const lastItem = getByTestId(`${STORY_TEST_IDS.fieldCombo.droplistItem}-3`);
