    ...
    <div className={cn(styles.root, className)} {...extractSupportProps(rest)} data-test-id={TEST_IDS.header}>
      {hasHeadline && (
        <div
          ...
          // Сторона с кнопкой 48×48 → отступ на самом wrapper'е (4); сторона без кнопки → отступ 16
          // даёт `.headline` (Figma topBar: control=no/before/after/beforeAndAfter). См. styles.module.scss.
        ...
        >
          {onBackButtonClick && (
            <Button
              ...
              size='m'
              icon={<ArrowLeftSVG />}
              // Icon-only кнопка → задаём accessible name явно (иначе screen reader озвучит «button»).
              aria-label='Назад'
