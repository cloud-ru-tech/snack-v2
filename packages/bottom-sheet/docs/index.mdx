...
import { WithSubHeadline } from '../demos/examples/WithSubHeadline'
import WithSubHeadlineSrc from '../demos/examples/WithSubHeadline.tsx?raw'
...
<Example title='С subHeadline' description='SearchBar / SegmentControl под headline-строкой' code={WithSubHeadlineSrc}>
  <WithSubHeadline client:visible />
</Example>
...
<Example title='Фильтры' description='Back-кнопка + подсказка, chips в subHeadline, SegmentControl и переключатели, «Применить / Сбросить»' code={FiltersSrc}>
...
<Example title='Picker тегов' description='Поиск в subHeadline фильтрует сетку тегов; клик переключает выбор' code={TagPickerSrc}>
