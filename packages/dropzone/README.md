...
#### Содержимое зоны
`children` — слот целиком: компонент задаёт только раскладку (центрирование и отступы по размеру), но не типографику и не цвет текста. Содержимое наследует стили родителя, поэтому шрифт и цвет задаёт место использования — обёрткой с нужным классом либо компонентом `Typography`.
