...
export function useGetViewDate(referenceDate: Date, viewMode: ViewMode, viewShift: number) {
  return useMemo(() => {
    switch (viewMode) {
      case VIEW_MODE.Decade: {
        const decadeFirstYear = Math.floor(referenceDate.getFullYear() / 10) * 10;
        return new Date(decadeFirstYear + viewShift * 10, 0, 1);
      }
      case VIEW_MODE.Year:
        return new Date(referenceDate.getFullYear() + viewShift, 0, 1);
      case VIEW_MODE.Month:
      default:
        return new Date(referenceDate.getFullYear(), referenceDate.getMonth() + viewShift, 1);
    }
  }, [referenceDate, viewMode, viewShift]);
}
