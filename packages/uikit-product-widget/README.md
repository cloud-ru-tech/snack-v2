...
Меню обоих вариантов рендерит `Droplist` из `@ds/list`: на mobile список открывается в `BottomSheet`, на desktop — анкорным popover'ом. Раскладка берётся из `AdaptiveProvider`.
