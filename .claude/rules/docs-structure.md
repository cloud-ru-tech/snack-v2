...
Внутри домена пакеты бьются на категории — под-группы сайдбара доки и Storybook, секции главной и разделы `/llms.txt`. Иерархия: домен-блок → категория → пакет. В отличие от доменов, категория **не** выводится из имени — она задаётся вручную в `apps/docs/src/config/categories.ts` (`CATEGORIES_BY_DOMAIN`). Домен без записи в реестре рендерит пакеты плоско.

**При добавлении пакета** в домен с категориями (`components`, `uikit-product`, `ai`) — впиши его имя (без `@ds/`-скоупа) в `packages` нужной категории его домена:

```ts
// apps/docs/src/config/categories.ts
CATEGORIES_BY_DOMAIN = {
  components: [
    { id: 'data-display', label: 'Data display', description: '…', packages: ['table', 'list', /* + новый пакет */] },
    // …
  ],
};
```

Если подходящей категории нет — заведи новую запись (`id` / `label` / `description` / `packages`). Не попавший в категорию пакет уходит в «Other» (build-warn). Порядок категорий в массиве = порядок в сайдбаре и на главной.
...
- **Триггеры/кнопки в демо — `Button` из `@ds/button`, не нативный `<button>`.** То же правило, что для trigger-based stories ([trigger-based-stories.md](./trigger-based-stories.md) §3): единообразие + корректные focus-visible / disabled / loading из коробки. Демо — копируемый потребителем код, поэтому моделирует best-practice DS.
...
- **Portals → НЕ оборачивать в `PortalContextProvider`**. Компоненты с порталом (`Tooltip`, `QuestionTooltip`, `Popover`, `Dropdown`, `Modal`, `Drawer`, любой `disabledToggleTip`/`tip`-проп) работают в демо **без** обёртки. Портал доков объявляет глобальный корень `#ds-portal-root` один раз (`apps/docs/src/components/layout/PortalRootApplier.tsx` → `getGlobalPortalRoot().current`); это `Symbol.for`-синглтон из `@ds/portal-context`, общий для всех независимых React-island'ов Astro, поэтому потребитель без своего провайдера монтирует портал туда. Контейнер лежит выше хрома доков (z-index `$docs-z-portal`), оверлеи перекрывают хедер/сайдбар. **Не добавляй `<PortalContextProvider>` в демо** — это лишний шум в `?raw`-листинге (потребителю он тоже не нужен: в реальном приложении корень объявляет оболочка). Исключение — демонстрация самого API кастомного `root` (`portal-context/CustomRoot`) или мобильный фрейм с собственным таргетом (`*/MobilePreview` с `safeAreaRef`): там `root={ref}` осмыслен и остаётся.
