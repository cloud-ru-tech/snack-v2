...
Группа (домен) в сайдбаре docs определяется автоматически по префиксу имени пакета — отдельных wire-действий не требуется. Список префиксов и их человекочитаемые лейблы — в конфиге доменов; новый префикс заводится туда же одной записью.

**Категория внутри домена — ручной шаг.** Для пакета в домене с категориями (`components`, `uikit-product`, `ai`) добавь его имя (без `@ds/`-скоупа) в `apps/docs/src/config/categories.ts` (`CATEGORIES_BY_DOMAIN`), иначе он упадёт в «Other» (build-warn). Формат и детали — в [docs-structure.md](./docs-structure.md) §«Категории внутри домена».
