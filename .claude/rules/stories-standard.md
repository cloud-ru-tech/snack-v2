...
- Покрывает **все** публичные оси из `constants.ts` × состояния (`disabled`, `loading`, `empty`). Каждая ось Figma-мастера — строка либо колонка (кроме осей из [figma-integration](./figma-integration.md) §«Оси Figma без пропа»).
