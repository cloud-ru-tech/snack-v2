...
> При миграции компонентного пакета из `storybook/packages/<pkg>/src` в `packages/<pkg>/src` **не уплощай** вложенность. Перенос — 1:1 по файлам. Меняется только скоуп импортов (`@design-system/*` → `@ds/*`) и путь Figma-токенов (легаси `@cloud-ru/figma-tokens-*`, например `@cloud-ru/figma-tokens-web` / `-cloud-platform` / `-giga-id`, → вендоренный `@ds/figma-variables`).
