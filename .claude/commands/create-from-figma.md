...
9. **Wire-точки** — чеклист: корневой `tsconfig.json::references`, `packages/tsconfig.esm.json` / `.cjs.json::references`, `apps/storybook/.storybook/main.ts::aliases`, `apps/storybook/package.json::deps`. (Aliases для docs подхватываются автоматически.) Совет: `pnpm add-package <pkg>` делает большинство wire'ов автоматически.
