...
**Триггеры:** «ответь на ревью», «забери комментарии из MR», «оставь комментарии в MR», «обсуди ревью», «закрой ниты в MR», «закрой nit в MR», «прокомментируй diff», «GitLab MR <number>».

Скилл — для работы с комментариями GitLab MR через инструменты `scripts/mr-comments/*.mts` (см. [scripts/mr-comments/README.md](../../scripts/mr-comments/README.md)). Агент не ходит в Web UI и не пишет SQL — он использует CLI: `fetch` → review локально → `post` / `reply`.

## Когда использовать

- В чате упомянут URL GitLab MR (`.../merge_requests/<N>`) или фраза «MR !N» / «ревью моего MR» / «ответь ревьюеру».
- Нужно автоматически разобрать чужие комментарии, подготовить ответы, и/или оставить серию инлайн-комментариев на конкретные строки diff'а.
...
### A. Ответ на чужие комментарии (review reply)
...
### B. Свой review: серия инлайн-комментариев на diff
...
- **Не создавай тред без привязки**, если можно привязать к строке. Комментарии без привязки к коду тяжело отсматривать.
...
- ❌ Удалять `sent_at`/`discussion_id` руками ради «переотправить» — лучше `delete-and-reset.mts`, он и треды снесёт, и локальное состояние приведёт в порядок.
